...
export function PricingCard({ tier, showFeatures = false }: PricingCardProps) {
  const { t } = useTranslation();
  const [open, setOpen] = useState(showFeatures);
    ...
    <div className="spot" style={spot} onMouseMove={onMove} onMouseLeave={onLeave}>
      ...
      >
        ...
        <div className="price-row">
          {tier.priceLine ? (
            <span className="amt pkg-scope">{label(tier.priceLine)}</span>
          ) : (
            <>
        ...
        {tier.footnotes?.map((note) => (
          <p key={isBilingual(note) ? note.en : note} className="rate-note" style={{ marginTop: 14 }}>
            {label(note)}
          </p>
        ))}
      </div>

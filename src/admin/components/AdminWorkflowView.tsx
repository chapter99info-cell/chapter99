...
/** Internal-only agency workflow reference — not shown on public site */
    ...
    <div>
      <div className="mb-12 grid grid-cols-1 gap-8 md:grid-cols-[0.95fr_1.05fr] md:items-end">
        <div>
          ...
          <h2 className="max-w-xl text-3xl font-semibold leading-tight tracking-[-0.03em] text-[#1A1A1A] sm:text-4xl">
            Ship fast.
            <br />
            Review first.
          </h2>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        {roleCards.map(({ role, title, summary, boundary, icon: Icon }) => (
          <article key={role} className="rounded-2xl border border-black/10 bg-white p-6">
            <div className="mb-6 flex h-11 w-11 items-center justify-center rounded-full bg-[#F8F5F0] text-[#1A1A1A] shadow-sm">
      ...
      <div className="mt-6 rounded-3xl border border-black/10 bg-white p-6 sm:p-8">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {ideaCards.map((idea) => (
            <article key={idea.title} className="rounded-2xl bg-[#F8F5F0] p-5">

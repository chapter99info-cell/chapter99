-- Add optional Pre-Wedding date (nullable) for bundled Pre-Wedding + Wedding Day bookings.
-- Same client, two shoot dates: dateISO stays the Wedding Day; pre_wedding_date_iso is the
-- second, earlier date. NULL for normal single-shoot clients (fully backward compatible).
alter table public.pm_clients add column if not exists pre_wedding_date_iso date;
